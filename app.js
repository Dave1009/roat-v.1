...
function addBoxes(row, column) {
    ...
    for (let i = 0; i < row * column; i++) {
        $("body").append(`<img src="img/field${Math.floor(Math.random() * 3 + 1)}.png" id="box_${i}" class="box_svg">`)
    }
...
function moveBlock(cordinatx, cordinaty, anime) {
    ...
    let moveBlockHeight = $("#moveBlock").height();
    let node = $(`[positionx=${cordinatx}][positiony=${cordinaty}]`)
    console.log(($(node).width() - width) / 2)
    let moveBlockTopChange = (width / 2)
